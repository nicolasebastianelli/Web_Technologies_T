...
public class Db2StudentCourseMappingDAO implements CourseStudentMappingDAO {
	...
	static String read_by_id_course = 
			"SELECT * " +
					"FROM students INNER JOIN courses_students on courses_students."+ID_S+" = students.id " +
					"WHERE " + ID_C + " = ? "
					;
	...
	static String read_by_id_student = 
			"SELECT * " +
					"FROM courses INNER JOIN courses_students on courses_students."+ID_C+" = courses.id " +
					"WHERE " + ID_S + " = ? "
					;
	...
	public List<StudentDTO> findStudentsByCourseId(int idCourse) {
		...
		try {
			...
			while ( rs.next() ) {
				...
				entry.setBirthDate(rs.getDate("birthdate"));
				result.add(entry);
			}
			rs.close();
			prep_stmt.close();
