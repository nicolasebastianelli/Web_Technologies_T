function main() {
	document.forms.partecipanti.hidden = false;
}

var Evento = {m:[],f:[]}

function validateForm(form) {
    if (isBlank(form.elements.partecipanti)) {
       alert("form vuoto");
       return;
    }
    Evento.m = [];
    Evento.f = [];
    var partecipanti = form.elements.partecipanti.value.split(",");
    for (var i = 0;i<partecipanti.length;i++){
    	var p = partecipanti[i].trim();
    	if (/[A-Z]([a-z])*/.test(p)){
    		if (p.endsWith("a"))
    			Evento.f.push(p);
    		else if (p.endsWith("e")||p.endsWith("o"))
    			Evento.m.push(p);
    		else {
    			alert("Error: " + p);	
    			return;
    ...
    myGetElementById("Tot").innerText = Evento.m.length + Evento.f.length;
    myGetElementById("M").innerText = Evento.m.length;
    myGetElementById("F").innerText = Evento.f.length;
    myGetElementById("list").innerText="";
    for (var i = 0;i<partecipanti.length;i++){
    	var p = partecipanti[i].trim();
    	myGetElementById("list").innerHTML += "<li>"+p+"<li/";
    }
    myGetElementById("results").hidden = false;
}
...
function checkLogin(field) {
    ...
    for (var i = 0; i < stringa.length; i++) {
        var c = stringa[i];
        if (!(c >= '0' && c <= '9') && stringavalida.length < 12) {
            stringavalida += c;
        }
    }
    field.value = stringavalida;
}
