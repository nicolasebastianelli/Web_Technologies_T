...
public class Db2FooDAO implements FooDAO {

	// --- Final Strings ---

	public static final String TABLE = "foo";

	public static final String ID = "id_foo";
	...
	public void create(FooDTO foo) {
		...
		try {
			...
			prep_stmt.setInt(1, foo.getId());
			prep_stmt.setString(2, foo.getValue());
			//TODO
	...
	public FooDTO read(int id) {
		...
		try {
			...
			if ( rs.next() ) {
				FooDTO entry = new Db2FooProxyDTO();
	...
	public boolean update(FooDTO foo) {
		...
		Connection conn = Db2DAOFactory.createConnection();
		try {
			PreparedStatement prep_stmt = conn.prepareStatement(Db2FooDAO.update);
			prep_stmt.clearParameters();
			prep_stmt.setString(1, foo.getValue());
			//TODO
			prep_stmt.setInt(2, foo.getId()); //Note: edit the value TODO
			prep_stmt.executeUpdate();
			result = true;
			prep_stmt.close();
		}
	...
	public boolean delete(int id) {
		...
		Connection conn = Db2DAOFactory.createConnection();
		try {
			PreparedStatement prep_stmt = conn.prepareStatement(Db2FooDAO.delete);
			prep_stmt.clearParameters();
