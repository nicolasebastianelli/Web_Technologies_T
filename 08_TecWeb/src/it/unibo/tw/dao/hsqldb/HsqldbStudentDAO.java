...
public class HsqldbStudentDAO implements StudentDAO {
	...
	public void create(StudentDTO t) {
		...
		// --- 3. Apertura della connessione ---
		Connection conn = HsqldbDAOFactory.createConnection();
		// --- 4. Tentativo di accesso al db e impostazione del risultato ---
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(HsqldbStudentDAO.insert);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(HsqldbStudentDAO.lastInsert);
			if ( rs.next() ) {
				result = rs.getLong(1);
			}		
			rs.close();
	...
	public StudentDTO read(int code) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(HsqldbStudentDAO.read_by_code);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
	...
	public boolean update(StudentDTO t) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(HsqldbStudentDAO.update);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setString(1, t.getFirstName());
			prep_stmt.setString(2, t.getLastName());
	...
	public boolean delete(int code) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(HsqldbStudentDAO.delete);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
			prep_stmt.setLong(1, code);
			// --- c. Esegui l'azione sul database ed estrai il risultato (se atteso)
	...
	public List<StudentDTO> findStudentByLastName(String lastName) {
		...
		try {
			// --- a. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(HsqldbStudentDAO.find_student_by_lastname);
			// --- b. Pulisci e imposta i parametri (se ve ne sono)
			prep_stmt.clearParameters();
